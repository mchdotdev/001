...
const logger = new Logger('/API/ADMIN/DELETE');
    ...
    case 'DELETE':
      {
        ...
        try {
          logger.log(`Deleting ${user.name}'s account...`);
          ...
          logger.error(
            `An error occured trying to delete ${user.name}'s (${
              user.userId
            }) account: ${error instanceof Error ? error.message : error}`,
          );
          res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({
            error: true,
            message: `An error occured trying to delete user's account: ${
