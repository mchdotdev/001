...
const logger = new Logger('/API/ADMIN/VERIFY');
    ...
    case 'PUT':
      {
        ...
        try {
          user.verified = true;
          await user.save();
          logger.debug(`Verified user ${user.name} (${user.userId})`);
          ...
          logger.error(
            `An error occured trying to verify user\nUser's info: ${
              user.name
            } ${user.userId}: ${
          ...
          res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({
            error: true,
            message: `An error occured trying to verify user: ${
              error instanceof Error ? error.message : error
            }`,
          });
        }
